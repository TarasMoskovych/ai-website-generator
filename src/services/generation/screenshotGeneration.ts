...
/**
 * Generates website HTML and CSS from a screenshot image using Claude's vision capabilities with streaming.
 *
 ...
 */
export async function* generateWebsiteFromImageStream(
  imageBase64: string,
  ...
  try {
    ...
    // Stream from Claude API with vision
    ...
    if (error instanceof Error) {
      if (error.name === 'AbortError' || controller.signal.aborted) {
        if (signal?.aborted) {
          ...
          yield { type: 'error', error: 'Request timed out after 2 minutes. Please try again with a simpler image.' };
        }
        return;
      }
      ...
      yield { type: 'error', error: `Failed to generate website from screenshot: ${error.message}` };
      return;
    }

    yield { type: 'error', error: 'Failed to generate website from screenshot: An unknown error occurred' };
  }
}
  ...
  try {
    ...
    if (error instanceof Error) {
      if (error.name === 'AbortError' || controller.signal.aborted) {
        ...
        throw new ScreenshotGenerationError(
          'Request timed out after 2 minutes. Please try again with a simpler image.',
